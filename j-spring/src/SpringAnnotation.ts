import {
  triggerClassAnnotation,
  triggerFieldAnnotation,
  triggerMethodAnnotation,
  triggerParamterAnnotation,
} from './SpringContext';
 ...
 */
export function classAnnotationGenerator(
  annoName: string,
  param?: AnnoParam,
  ref?: Function
): Function {
  return function temp<T extends Clazz>(constructor: T) {
    triggerClassAnnotation(
      constructor,
      new Anntation(annoName, ref ? ref : temp, param)
    );
...
export function fieldAnnotationGenerator(
  annoName: string,
  param: AnnoParam,
  ref?: Function
): Function {
  return function temp(_target: any, key: string) {
    (param as ReflectParam).reflectType = Reflect.getMetadata(
      'design:type',
      _target,
      key
    );
...
export function methodAnnotationGenerator(
  annoName: string,
  param: AnnoParam,
  ref?: Function
): Function {
...
export function paramterAnnotationGenerator(
  annoName: string,
  paramterName: string,
  param: AnnoParam,
  ref?: Function
...
export const Component = () =>
  classAnnotationGenerator('j-spring.Component', {}, Component);

function setForceDefaultValue(param: { force?: boolean }) {
  if (param.force === void 0) param.force = true;
}
...
export const Method = () =>
  methodAnnotationGenerator('j-spring.Method', {}, Method);

export const Paramter = (name: string) =>
  paramterAnnotationGenerator('j-spring.Paramter', name, {}, Paramter);
