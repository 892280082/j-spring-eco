...
function isSpringStarter(starter: SpringStarter): boolean {
  return (
    starter &&
    isFunction(starter.doStart) &&
    isFunction(starter.isSpringStater) &&
    starter?.isSpringStater()
  );
}
...
function isBeanPostProcessorClass(clazz: Clazz): boolean {
  ...
  return (
    isFunction(post.getSort) &&
    isFunction(post.postProcessAfterInitialization) &&
    isFunction(post.postProcessBeforeInitialization)
  );
}

//根据类获取定义
export const getBeanDefineByClass = function(
  clazz: Clazz
): BeanDefine | undefined {
  return Array.from(beanDefineList).find(b => b.clazz == clazz);
};

//替换类
export function replaceClazz<T, E extends T>(
  clazz: new () => T,
  target: new () => E
): number {
  const fieldDefineList: FieldDefine[] = Array.from(beanDefineList)
    .map(bd => bd.fieldList)
    .reduce((s, v) => s.concat(v), []);
  const allFieldAnnotation: Anntation[] = fieldDefineList
    .map(f => f.annotationList)
    .reduce((s, v) => s.concat(v), []);

  const matchAnnotation = allFieldAnnotation
    .filter(a => a.clazz === Autowired)
    .filter(a => (a.params as AutowiredParam<any>).clazz == clazz);

  if (matchAnnotation.length == 0)
    throw Error(`[ReplaceClazzError] not match class ${clazz}`);

  matchAnnotation.forEach(
    a => ((a.params as AutowiredParam<any>).clazz = target)
  );
  ...
  if (needAutowiredList.length > 0) {
    ...
    needAutowiredList.forEach(at => {
      ...
      if (findBean) {
        ...
        logger(
          `序号:${getCount++} 延迟装配 [命中] 类：${clazz?.name} 字段：${
            fieldBd.name
          }`
        );
        ...
        if (!force && selfBean[fieldBd.name] !== void 0) {
          ...
          at.state = 1;
          logger(
            `序号:${getCount++} 延迟装配 [默认值] 类:${bd.clazz.name} 字段：${
              fieldBd.name
            }`
          );
        }
...
export const assemble = function(clazz: Clazz) {
  ...
  if (!bd) {
    throw Error(
      `找不到类:[${clazz.name}]的BeanDefine信息,是否忘记在该类上添加 @Component() 装饰器?`
    );
  }
...
function getSoredBeanPostProcessorList(): BeanPostProcessor[] {
  return Array.from(beanPostProcessorList).sort(
    (v1, v2) => v1.getSort() - v2.getSort()
  );
}
...
function assembleBeanDefine(bd: BeanDefine): any {
  ...
  //bean处理器处理装配前操作
  beanPostProcessor.forEach(
    post => (bean = post.postProcessBeforeInitialization(bean, bd))
  );
      ...
      if (anno.clazz === Value) {
        ...
        if (!param.force && !hasConfig(param.path)) {
          if (bean[fieldName] === void 0) {
            throw Error(
              `类:[${bd.clazz.name}] 字段:${fieldName} 若无配置则必须设置默认值!`
            );
          }

          logger(
            `${bd.clazz.name} <= @Value[默认值] 字段：${fieldName} 路径:${param.path} 值:${bean[fieldName]} `
          );
        } else {
          const value = geFormatValue(
            param.path,
            (anno.params as ReflectParam).reflectType
          );
          if (!Reflect.set(bean, fieldName, value)) {
            throw Error(`[反射装配错误] 类:${bd.clazz.name} 字段:${fieldName}`);
          }
          logger(
            `${bd.clazz.name} <= @Value[配置] 字段：${fieldName} 路径:${param.path}  值:${value} `
          );
        }
      }
    });
  ...
  //bean处理器处理装配后操作
  beanPostProcessor.forEach(
    post => (bean = post.postProcessAfterInitialization(bean, bd))
  );
...
const instanceBeanPostProcessor = () => {
  logger(
    '实例化后置处理器。 数量：' + bindBeanProcessorClazzList.size + ' (1/2) '
  );
  let index = 1;
  Array.from(bindBeanProcessorClazzList)
    .map(clazz => {
  ...
  beanDefineList.forEach(bd => {
    ...
    if (fdList.length > 0) {
      ...
      fdList.forEach(fd => {
        ...
        const configValue = hasConfig(param.path)
          ? geFormatValue(param.path, String)
          : '无';

        const printMsg = `路径:${
          param.path
        }  备注:${remark} 值:${configValue}  强制:${
          param.force ? '是' : '否'
        }  默认值:${defaultValue}`;
...
export const beanFactoryInit = () => {
  ...
  logger(
    `内置:j-spring.log.fileName  备注:winston日志输出文件 值:否 强制：否 默认值:无 `
  );
  printConfig();
