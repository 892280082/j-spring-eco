import {Autowired, Clazz, Component, isFunction, spring, SpringFactoryBean, SpringStarter } from '../src'
...
describe('test async assmble by type',()=>{
    ...
    it('async load',()=>{
        ...
        function isSquery(bean:SqlQuery):boolean {
            return bean && isFunction(bean.query) && isFunction(bean.isSqlQuery) && bean.isSqlQuery();
        }
        ...
        class DbStarter implements SpringStarter {
            async doStart(_clazzMap: Map<Clazz, any>): Promise<any> {
        ...
        spring.bindList([SqliteQueryBeanFactory,DbStarter]).launchAsync(Application).then(v => expect(v).toEqual(['query ok']))
    })
