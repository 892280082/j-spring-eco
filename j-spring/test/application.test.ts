import { Autowired, Component, launch, launchAsync, spring,SpringStarter, Value  } from '../src';
...
describe('application test!', () => {
  ...
  it('run starter applicaiton by spring',()=>{
    ...
    spring.starter(Application).launchAsync(Application).then(v => expect(v).toBe(100))
  ...
  it('run starter applicaiton by spring and inject more starter ',()=>{
    ...
    spring.starter(Db,Server).launchAsync(Application).then(v => expect(v).toBe(`db ok server ok`))
  ...
  it('only run starter',()=>{
    ...
    spring.bind(Db).invokeStarter().then(_v => expect(count).toBe(100))
